...
void ADCInit(uint32_t RCC_APB2Periph,uint32_t RCC_AHB1Periph,ADC_TypeDef* ADCx, uint8_t ADC_Channel,GPIO_TypeDef* GPIOx, uint16_t GPIO_Pin)
{
	ADC_InitTypeDef ADC_init_structure; //Structure for adc confguration
	 ...
	 //Clock configuration
	 RCC_APB2PeriphClockCmd(RCC_APB2Periph,ENABLE);//The ADC1 is connected the APB2 peripheral bus thus we will use its clock source
	 RCC_AHB1PeriphClockCmd(RCC_AHB1Periph,ENABLE);//Clock for the ADC port!! Do not forget about this one ;)
	 //Analog pin configuration
	 GPIO_initStructre.GPIO_Pin = GPIO_Pin;//The channel 10 is connected to PC0
	 GPIO_initStructre.GPIO_Mode = GPIO_Mode_AN; //The PC0 pin is configured in analog mode
	 GPIO_initStructre.GPIO_PuPd = GPIO_PuPd_NOPULL; //We don't need any pull up or pull down
	 GPIO_Init(GPIOx,&GPIO_initStructre);//Affecting the port with the initialization structure configuration
