								...
								uint16_t GPIO_PinSourceSDA,uint8_t GPIO_AF, I2C_TypeDef* I2Cx)
{
	 ...
	 */
	GPIO_InitStruct.GPIO_Pin = GPIO_Pin; // we are going to use PB6 and PB7
	GPIO_InitStruct.GPIO_Mode = GPIO_Mode_AF;			// set pins to alternate function
	GPIO_InitStruct.GPIO_Speed = GPIO_Speed_50MHz;		// set GPIO speed
	GPIO_InitStruct.GPIO_OType = GPIO_OType_OD;			// set output to open drain --> the line has to be only pulled low, not driven high
	GPIO_InitStruct.GPIO_PuPd = GPIO_PuPd_UP;			// enable pull up resistors
	GPIO_Init(GPIOx, &GPIO_InitStruct);					// init GPIOB
	
	// Connect I2C1 pins to AF  
	GPIO_PinAFConfig(GPIOx, GPIO_PinSourceSCL, GPIO_AF);	// SCL
	GPIO_PinAFConfig(GPIOx, GPIO_PinSourceSDA, GPIO_AF); // SDA
